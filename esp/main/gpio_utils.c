...
void init_gpios() {
    ...
    init_gpio(GPIO_NUM_0, GPIO_MODE_OUTPUT, HIGH_LEVEL, OFF);
    //init_gpio(GPIO_NUM_2, GPIO_MODE_OUTPUT, LOW_LEVEL, OFF);
    //init_gpio(GPIO_NUM_4, GPIO_MODE_INPUT, HIGH_LEVEL, OFF);
    //init_gpio(GPIO_NUM_5, GPIO_MODE_INPUT, HIGH_LEVEL, OFF);
    ...
    //gpio_set_intr_type(gpio[4].pin, GPIO_INTR_POSEDGE);
    //gpio_isr_register(gpio[4].pin, GPIO_INTR_POSEDGE);
    //gpio_install_isr_service(gpio[4].pin, GPIO_INTR_POSEDGE);
...
esp_err_t set_gpio(uint8_t pin, uint8_t _st, uint8_t mqtt_send_en){
    ...
    if ( _st > 1 ) {
        //ESP_LOGI(TAG, "%s: FAIL: state (%d) > 1", __func__, _st);
        return ESP_FAIL;
    }
    ...
    // check current state
    //ESP_LOGI(TAG, "%s: check current state....", __func__);
    if ( gpio[i].state == _st) {
        //ESP_LOGI(TAG, "%s: gpio%d has the same state. BREAK!!!", __func__, pin);
        return ESP_OK;
    }
    ...
    //ESP_LOGI(TAG, "%s: gpio%d has state %d switch to new state %d", __func__, pin, gpio[i].state, _st);
