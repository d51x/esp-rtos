...
static TaskHandle_t xHanldeGPIOState = NULL;
static TaskHandle_t xHanldeAll = NULL;
static QueueHandle_t xQueueGPIOState = NULL;
...
static void mqtt_receive_message_task(void *arg);
...
//    int msg_id;
    ...
    switch (event->event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            // create tasks
            // create task to receive mesages of gpio state
            //mqtt_uninitialize();
            xTaskCreate( mqtt_receive_message_task, "mqtt_gpio", 1000, NULL, 10, &xHanldeGPIOState );
...
static void mqtt_uninitialize(){
    ...
    if( xHanldeGPIOState != NULL ) {
        ESP_LOGI(TAG, "xHanldeGPIOState is not NULL, delete task");
        vTaskDelete( xHanldeGPIOState );
        xHanldeGPIOState = NULL;
    }  else {
        ESP_LOGI(TAG, "xHanldeGPIOState is NULL");
    }

    if ( xQueueGPIOState != NULL  ) {
        ESP_LOGI(TAG, "xQueueGPIOState is not NULL, delete queue");
        vQueueDelete(xQueueGPIOState);
        xQueueGPIOState = NULL;
    } else {
        ESP_LOGI(TAG, "xQueueGPIOState is NULL");
    }
}
...
static void mqtt_receive_message_task(void *arg){
    ESP_LOGI(TAG, "%s started...", __func__);
    if (xQueueGPIOState == NULL ) {
        ESP_LOGI(TAG, "Create xQueueGPIOState");
        xQueueGPIOState = xQueueCreate( 30, sizeof( gpio_state_msg_t ) );
        ESP_LOGI(TAG, "xQueueGPIOState is %p", (int*)xQueueGPIOState);
    }

    // transfer xqueue to gpio handler
    set_queue_gpio_state(xQueueGPIOState);
    ...
    for( ;; ) {
        ...
        if ( xQueueReceive( xQueueGPIOState, &mqtt_gpio, (portTickType)portMAX_DELAY /*xTicksToWait*/ ) == pdPASS ) {
            ESP_LOGI(TAG, "xQueueGPIOState receive mqtt_gpio: pin: %d, state: %d, mode: %d", mqtt_gpio.pin, mqtt_gpio.state, mqtt_gpio.mode);
            if ( mqtt_gpio.mode == GPIO_MODE_INPUT ) {
                ESP_LOGD(TAG, "xQueueGPIOState publish INPUT gpio state");
                mqtt_publish_gpio_input_state(&mqtt_gpio);
            } else if ( mqtt_gpio.mode == GPIO_MODE_OUTPUT ) {
                ESP_LOGI(TAG, "xQueueGPIOState publish OUTPUT gpio state");
                mqtt_publish_gpio_output_state(&mqtt_gpio);
            }
        }
        taskYIELD();
    }
...
void mqtt_publish_gpio_input_state(const gpio_state_msg_t *gpio){
    ESP_LOGD(TAG, "%s started", __func__);
    char topic[32] = MQTT_DEVICE_GPIO_INPUT;
    itoa(gpio->pin, topic+strlen(MQTT_DEVICE_GPIO_INPUT), 10);
...
void mqtt_publish_gpio_output_state(const gpio_state_msg_t *gpio){
    ESP_LOGI(TAG, "%s started", __func__);
    char topic[32] = MQTT_DEVICE_GPIO_OUTPUT;
    itoa(gpio->pin, topic+strlen(MQTT_DEVICE_GPIO_OUTPUT), 10);
...
#ifdef DHT   //TODO: учесть несколько DHT
void mqtt_publish_dhtt(){
    char payload[10];
    sprintf(payload,"%.2f",  dht.temp);
    mqtt_publish_generic( MQTT_DEVICE_DHTT, payload);     
}

void mqtt_publish_dhth(){
    char payload[10];
    sprintf(payload,"%.2f",  dht.hum);
    mqtt_publish_generic( MQTT_DEVICE_DHTH, payload);    
}
#endif
    ...
    void mqtt_publish_dsw(){
        ...
        for (uint8_t i=0;i<DSW_COUNT;i++){
            if ( ds18b20[i].addr[0] ) {
                sprintf(topic, MQTT_DEVICE_DSW, i+1);
...
void mqtt_publish_all_gpio_state() {
    uint8_t i;
    for ( i = 0; i < GPIO_NUM_MAX; i++) {
        ...
        mqtt_publish_gpio_output_state((gpio_state_msg_t *)&gpio[i]);
    }
}
...
static void process_data(esp_mqtt_event_handle_t event){
    ...
    ESP_LOGV(TAG, "DATA=%s", data);

    char *istr;
    istr = strstr(topic, MQTT_DEVICE_GPIO_OUTPUT);
    if ( istr != NULL ) {
        ESP_LOGV(TAG, "%s: ---------------------------------------------", __func__);
        ESP_LOGV(TAG, "%s: gpio output topic found,  pos %s", __func__, istr);
        char num[3]="";
        ...
        trim(num);
        ESP_LOGV(TAG, "%s: gpio index %s", __func__, num);
        trim(data);
        ESP_LOGV(TAG, "%s: gpio state %s", __func__, data);
        
        uint8_t pin, st;
        if ( str_to_uint8(&pin, num, 10) == 0 && str_to_uint8(&st, data, 10) == 0) {
            ESP_LOGV(TAG, "%s: set GPIO pin %d to %d", __func__, pin, st);
            set_gpio(pin, st);
        } else {
            ESP_LOGV(TAG, "%s: FAIL get pin or st", __func__);
        }          
    } 
    free(topic);
    free(data);
