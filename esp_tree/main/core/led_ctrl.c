...
int8_t get_effect_id(){
	return effect_id;
}

void get_effect_name(int8_t id, char *name){
	if ( id < 0 || id >= COLOR_EFFECTS_MAX) {
		strcpy(name, "Unknown effect");
	} else {
		strcpy(name, color_effects[id]);
	}
}

color_hsv_t get_current_color_hsv(){
	return __hsv;	
}
