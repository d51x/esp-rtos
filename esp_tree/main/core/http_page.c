...
void get_main_page_data(char *data) {
    ...
    // ============================= PRINT LEDCTRL INFO ==============================
    char effect[20];
    int8_t effect_id = get_effect_id();
    get_effect_name(effect_id, effect);
    color_hsv_t hsv = get_current_color_hsv();
    color_rgb_t rgb;
    hsv_to_rgb(&rgb, hsv);

    const char *ledctrl_data = "<div class=\"ledc\">"
                                "<p><span><b>Color effect:</b> %s (%d)</span></p>"
                                "<p><span><b>HSV color:</b> %d %d %d</span></p>"
                                "<p><span><b>RGB color:</b> %d %d %d</span></p>"
                                "</div>";

    sprintf(data+strlen(data), ledctrl_data, effect, effect_id, hsv.h, hsv.s, hsv.v, rgb.r, rgb.g, rgb.b);
    // ==============================================================================
